...
class GUI:
    ...
    def setup_button(self, x, y, text, option, image_path=None):
        ...
        if option == "INPUT_FILE" or option == "INPUT_FOLDER":
            self.button["command"] = partial(self.file.open_file_or_folder_dialog, option)
            self.button.dnd_bind('<<Drop>>', lambda e: self.file.display_input_tree(e.data))
